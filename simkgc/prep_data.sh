        ...
        for role in 0 1; do
            for causation in 0 1; do
                ...
                for syntax in ${syntax_list}; do
                    ...
                    if [ ! -d "~/data/SimKGC/NarrativeInductiveDataset/${name}" ]; then
                        python prep_data.py ~/data/SimKGC/NarrativeInductiveDataset/${name} --prop ${prop} --subevent ${subevent} --role ${role} --causation ${causation} --syntax ${syntax} --no-inductive-split --prep-simkgc
                    else
                        echo "Directory ~/data/SimKGC/NarrativeInductiveDataset/${name} already exists, skipping."
                    fi
